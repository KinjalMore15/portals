    ...
    <div class="row">
        ...
        <div class="col-md-7 col-sm-8">
            <div class="jobs-content">               
                ...
                @foreach($save_jobs as $save_job)
                  <div class="box">
                    <div class="text-box" style="float: right;">                    
                        ...
                        <h3 class="hideOverflow"><a href="{{ URL::to('/job_detail/'.$save_job->job->id) }}" style="text-decoration:none;">{{ $save_job->job->job_title }}</a></h3>
                       
                        <div class="clearfix"> 
                        @if(!empty($save_job->designation))
                        <i class="fa fa-briefcase"></i><b style="padding: 3px">{{ $save_job->job->designation }}</b> 
                        @endif
                        @if(!empty($save_job->city))
                        <i class="fa fa-map-marker"></i><b style="padding: 3px">{{ $save_job->job->city }}, {{ $save_job->job->state }}</b>
                        @endif
                        @if(!empty($save_job->min_salary))
                        <i class="fa fa-money"></i><b style="padding: 3px">${{ $save_job->job->min_salary }} - ${{ $save_job->job->max_salary }}</b>
                        @endif
                        </div>
                        <div class="description">
                        <p>{{ $save_job->job->job_description }} 
                        </p>
                        </div>
                        <span >Posted on:<i class="fa fa-calendar">{{$save_job->job->created_at}}</i> </span>

                    <div class="hoverWrapper">
                        <a href="{{ URL::to('/job_detail/'.$save_job->job->id) }}" class="btn-apply">apply</a>
                    </div>
                    </div>
                ...
                {{ $save_jobs->appends(\Request::except('page'))->render() }}
